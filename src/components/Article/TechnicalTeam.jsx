  ...
  return (
    <Grid
      ...
      <Typography variant="body" sx={{ width: "100%" }}>
        Technical team members are responsible for the development of the
        projects and the maintenance of the infrastructure.
      </Typography>
      {data && data.length > 0 ? (
        <>
          {data.map((member) => (
            <Grid item xs={12} sm={6} md={4} lg={3} xl={2} key={member.id}>
              <Tooltip
                title="Click on avatar to open in new browser tab"
                arrow
                placement="top">
                <Paper
                  elevation={3}
